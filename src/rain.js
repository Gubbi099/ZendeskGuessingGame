...
export const stopRain = () => {
  const thunderContainer = document.getElementById("thunderContainer");
  thunderContainer.classList.remove("show");

  setTimeout(() => thunderContainer.remove(), 3000);
};

export const startRain = () => {
  const thunderContainer = document.createElement("div");
  thunderContainer.setAttribute("id", "thunderContainer");
  thunderContainer.classList.add("thunderContainer");

  const thunder = document.createElement("div");
  thunder.classList.add("thunder");

  thunderContainer.appendChild(thunder);

  const canvas1 = document.createElement("canvas");
  canvas1.setAttribute("id", "canvas1");
  thunder.appendChild(canvas1);
  const canvas2 = document.createElement("canvas");
  canvas2.setAttribute("id", "canvas2");
  thunder.appendChild(canvas2);
  const canvas3 = document.createElement("canvas");
  canvas3.setAttribute("id", "canvas3");
  thunder.appendChild(canvas3);

  document.body.prepend(thunderContainer);

  setTimeout(() => thunderContainer.classList.add("show"), 100);
